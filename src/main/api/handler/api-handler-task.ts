...
export class TaskApiHandler extends ApiHandleStrategy implements IpcToolkitTaskApi {
  ...
  updateTask(context: ApiCallerContext, data: Pick<Task, 'id'> & TaskUpdate): Promise<void> {
  ...
  executeTask(context: ApiCallerContext, task: Task): Promise<TaskExecution> {
  ...
  abortTaskExecution(
    context: ApiCallerContext,
    taskExecutionId: number | TaskExecution,
    reason?: string,
  ): Promise<void> {
    return taskService.abortTaskExecution(taskExecutionId, reason)
  }
}
